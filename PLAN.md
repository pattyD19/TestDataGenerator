...
|---|---|---|
| **0. Spike** ✅ | Prove the two risky bits: 500 assets into iOS Photos via `PHAssetCreationRequest` with correct `creationDate`, and 500 into Android MediaStore with correct `DATE_TAKEN`. Nothing else matters if these are slow or lossy. **Never run as a spike** — the phases were built directly and both risks were retired by the real loaders on real hardware, which is stronger evidence than a throwaway would have been. Android: **3,466 assets** on a physical Galaxy S24, every one indexed, worst `DATE_TAKEN` delta 0.999 s. iOS: **900 assets** on a physical iPhone 15 Pro on 2026-09-03, 900/900 accepted at ~19 MB/s with nine `performChanges` batches and no stall — see `docs/conformance/iphone-15-pro-physical-900-assets.md`. Neither is slow; neither is lossy. | done |
| **1. CLI generator** ✅ | `tdg build --size 25GB --profile iphone-15-pro --out ./pack` → folder + `manifest.json` + `LICENSES.csv`. Amplifier, exact-size planner, hand-rolled EXIF writer, synthetic bootstrap seed pool. **JPEG + MP4 only.** The harvester runs: `tdg harvest` pulls real CC0/PD stills from Wikimedia (Openverse now answers 401 without a key) over stdlib `urllib`, and covers any shortfall from one source with the other. **Stills are held apart perceptually, not just by checksum** — a bigger pool, per-file crop/flip/rotation and colour jitter, and a dHash gate that redraws any still landing within 6 bits of one already in the pack. Twelve synthetic seeds gave 268 identical pairs in a 2,227-photo pack; 212 harvested seeds give none. Built and verified 2026-08-29 — see `packages/generator/`. | done |
...
| **3. Web control plane** ✅ | Browser front end + generator API. Presets, job creation, live progress over SSE, cancel/resume, LAN mode, per-job pairing code, and a pack API with `Range` support that loaders consume directly. **Built as stdlib Python + hand-written JS, not Next.js** — the generator is Python, so the server imports it and reads progress from the build's own checkpoint; no second runtime, no build step. QR pairing deferred until Phase 4/5 give it something to scan. **Prune added 2026-08-30**: a pack's media can be reclaimed while its row survives, so a pruned job still rebuilds byte for byte from its id and seed, and every layer — server, both loaders — reports `410 Gone` as a pruned pack rather than a transfer failure. Built 2026-08-29 — see `packages/web/`. | done |
| **4. Android loader** ✅ | Kotlin, minSdk 30, single screen: six-digit code → stream manifest → `MediaStore.insert()` with `IS_PENDING` → `DATE_TAKEN`/`RELATIVE_PATH`. Foreground service + per-job receipts. Plus wipe, which needs no storage permission because the app owns every file it wrote. Verified on a Pixel 9 emulator (Android 17): 24 assets indexed, photos and video, `DATE_TAKEN` exact, wipe clean. **Run on a physical Galaxy S24 (Android 16, One UI) on 2026-08-30** — 144/144 indexed, and a 10 GB / 3,466-file fill ran 14.9 minutes unplugged with the screen off without the foreground service being suspended. Wipe now removes the album directory it empties, not just the rows. **The Android 11 floor and the budget A-series remain unrun.** Built 2026-08-29 — see `clients/android/`. | done |
| **5. iOS loader** ✅ | SwiftUI, iOS 17 floor, same flow → batched `PHAssetCreationRequest` (100 per `performChanges`). Plus wipe. **Permission is staged: add-only to fill, escalating to full access only for wipe** — verified that `.addOnly` suffices for every creation including video. Verified on an iPhone 17 Pro simulator (iOS 26.5): 24 assets imported, `creationDate` exact to 0.000 s, wipe removed all 24 leaving the stock photos untouched. **Run on a physical iPhone 15 Pro (iOS 26.6.1) on 2026-08-30**, which found two loader bugs no simulator could: a single invalid asset failing the whole `performChanges` transaction, and `shouldMoveFile` consuming staged files so the retry misreported good assets. Both fixed. Still not near TestFlight — installation is by cable. Built 2026-08-29 — see `clients/ios/`. | done |
| **6. Conformance harness** ◐ | `tdg verify` asks the *device* what its gallery indexed rather than trusting the loader — assets indexed, capture time preserved, album grouping, fill duration — and exits non-zero on failure so it can gate CI. Results in `docs/conformance/`. **Four of six devices covered** — two emulated, plus a physical iPhone 15 Pro (iOS 26.6.1, 71/72 accepted) and a physical Galaxy S24 (Android 16, 144/144 indexed, wipe clean). **Samsung's battery manager is tested and passed**: a 10 GB, 3,466-file fill ran 15 minutes unplugged with the screen off without the foreground service being suspended. The Galaxy A-series and both OS floors remain unrun for want of hardware. A physical iPhone cannot be verified by `tdg verify` itself; the real-device run pulled the loader's receipt off the phone with `devicectl` instead. Built 2026-08-29 — see `packages/generator/tdg/verify.py`. | harness done, matrix partial |
| **7. Realism & formats** ◐ | **HEIC and HEVC done — the v1 format gap is closed.** `--photo-format jpeg|heic|mixed`, `--video-codec h264|hevc` (tagged `hvc1`), EXIF and the timezone offsets survive the HEIC conversion intact. `--edge-cases` adds a burst run, an exact duplicate, a screenshot with no EXIF, a non-ASCII filename, a truncated JPEG and a zero-byte file. Verified on both devices: MediaStore assigns `image/heic` correctly and indexes all of it. **Live-Photo pairs and the 4 GB single file are not built.** Built 2026-08-29. | formats done, some realism outstanding |
...
| Risk | Mitigation |
|---|---|
| iOS Photos import is slow at scale (10k+ assets) | **Narrowed 2026-09-03, not closed.** Batching at 100 per `performChanges` is in and measured: **900 assets into a physical iPhone 15 Pro at ~19 MB/s**, nine batches, no stall — faster than the same Wi-Fi managed to a Galaxy S24. Photos was not the bottleneck at that size. 10k+ is still unmeasured, and a 128 GB fill would still be an overnight run. |
...
| ~~Android suspends the loader mid-fill~~ | **Closed 2026-08-30.** Foreground service + ongoing notification, verified on a physical Galaxy S24: a 10 GB fill ran 15 minutes unplugged, screen off, `isForeground=true` throughout, flat throughput, no kill or ANR. A multi-hour fill is still untested. |
...
| ~~**v1 ships no HEIC/HEVC**~~ | **Closed 2026-08-29.** `--photo-format heic` and `--video-codec hevc` both land and index correctly on an Android 17 emulator and an iOS 26.5 simulator. HEIC needs `sips` (macOS) or `pip install pillow-heif` elsewhere. |
...
1. ~~Whether attribution-required media (Pexels, Blender) is acceptable inside shared test
   packs, or whether the seed pool should be **CC0-only**.~~ **Settled in the
   implementation:** `tdg harvest` takes CC0/public-domain only, and
   attribution-required licences need an explicit `--any-license`. Whether to
   ever pass that flag is now a per-pack decision rather than a design one.
